...
const Alumni = () => {
  ...
  const uniqueDepartments = [
    ...new Set(
      data?.flatMap((batch) => Object.keys(batch.department))
    ),
  ];

  // Filtering Logic
  const filteredData = data?.filter((batchData) => {
    const matchesBatch = selectedBatch ? batchData.batch === selectedBatch : true;
    ...
    const matchesSearch = searchTerm
      ? Object.values(batchData.department).some((students) =>
          students.some((student) =>
            [batchData.batch, ...Object.keys(batchData.department), student.name, student.number, student.email]
              .some((field) =>
    ...
    <div className="p-4">
      ...
      {filteredData?.map((batchData) => (
        <div key={batchData._id} className="mb-6 p-4 border rounded-lg shadow">
          <h2 className="text-xl font-semibold mb-2">Batch: {batchData.batch}</h2>

          {Object.entries(batchData.department)
            .filter(([deptName]) => (selectedDepartment ? deptName === selectedDepartment : true))
            .map(([deptName, students]) => (
              <div key={deptName} className="mb-4">
                <h3 className="text-lg font-medium text-blue-600">{deptName} Department</h3>
                  ...
                  {students
                    ...
                    .map((student, index) => (
                        ...
                        <div className="flex space-x-4">
                          {student.image ? (
                            <img src={student.image} alt={student.name} className="object-cover w-12 h-12 rounded-full shadow dark:bg-gray-500" />
                          ) : (
                            <div className="w-12 h-12 rounded-full bg-gray-300 flex items-center justify-center text-gray-600">N/A</div>
                          )}
                          <div className="flex flex-col space-y-1">
                            <p className="text-sm font-semibold">{student.name}</p>
                            <span className="text-xs dark:text-gray-600">{student.email}</span>
                          </div>
                        </div>
                        <div>
